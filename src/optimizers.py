...
def get_optimizer(stats, params):
    ...
    def optim_fn(optimizer_name):
        ...
        elif optimizer_name.lower() == "decoy":
    ...
    optimizer, optimizer_decoy = map(optim_fn, (optimizer_name, "decoy"))
    ...
    if is0(stats['distortion_nans']) and is0(stats['rate_nans']) and (grad_norm < params["skip_threshold"]):
        ...
        train_ops = optimizer_decoy.apply_gradients(list(zip(grads, variables)), global_step=global_step)
        #train_ops = tf.group([tf.assign_add(global_step, 1)])
        skipped_updates = one 
    for name in ['rate_nans', 'distortion_nans']:
        stats[name] = zero_or_one(stats[name])
